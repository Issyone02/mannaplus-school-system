...
export default function StudentNoticesPage() {
  ...
  const fetchNotices = async () => {
    try {
      ...
      // ✅ FIXED: Removed .eq('is_active', true) because the column doesn't exist
      const { data: noticesData, error } = await supabase
        .from('notices')
        .select('*')
        .or(`audience.eq.all,audience.eq.students,target_class.eq.${studentData.class_id}`)
          ...
          <div className="space-y-4">
            {notices.map((notice) => (
              <div key={notice.id} className={`bg-white rounded-xl shadow-md p-6 border-l-4 ${
                notice.is_urgent ? 'border-red-500' : 'border-orange-500'
              }`}>
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-full ${notice.is_urgent ? 'bg-red-100' : 'bg-orange-100'}`}>
                    {notice.is_urgent ? (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      {notice.audience && (
                        <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-bold rounded capitalize">
                          {notice.audience === 'all' ? 'Everyone' : 
                           notice.audience === 'students' ? 'Students' : 
                           notice.audience === 'parents' ? 'Parents' : 
                           notice.audience === 'teachers' ? 'Teachers' : 
                           notice.target_class ? `Class ${notice.target_class.toUpperCase()}` : notice.audience}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500 flex-wrap">
                      ...
                      {notice.target_class && notice.audience === 'specific_class' && (
                        <span>📚 Class: {notice.target_class.toUpperCase()}</span>
                      )}
                    </div>
                  </div>
                </div>
