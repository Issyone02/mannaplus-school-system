  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-0 mb-6 text-center md:text-left">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center justify-center md:justify-start gap-2"><MailOpen size={28} /> Enquiries Inbox</h1>
          <p className="text-gray-700">Messages sent from the public Contact page</p>
        </div>
        <button onClick={fetchEnquiries} className="flex items-center justify-center gap-2 bg-blue-600 text-white px-4 py-2 rounded font-bold hover:bg-blue-700 w-full md:w-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-5">
          {enquiries.map(e => (
            <div key={e.id} className="bg-white rounded-xl shadow border border-gray-200 p-4 md:p-6">
              <div className="flex justify-between items-start gap-2 mb-3 flex-wrap">
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-gray-900 text-base md:text-lg break-words">{e.full_name}</h3>
                  <p className="text-xs md:text-sm text-gray-600 break-all">{e.email}</p>
                  {e.phone && <p className="text-xs md:text-sm text-gray-600 flex items-center gap-1 mt-1"><Phone size={12} /> {e.phone}</p>}
                </div>
                <button onClick={() => handleDelete(e.id)} className="text-red-600 hover:text-red-800 flex-shrink-0" title="Delete"><Trash2 size={16} /></button>
              </div>
              ...
              <p className="text-gray-700 bg-gray-50 rounded-lg p-3 md:p-4 border border-gray-100 whitespace-pre-wrap text-sm md:text-base">{e.message}</p>
              <p className="text-xs text-gray-400 mt-2 md:mt-3">{new Date(e.created_at).toLocaleString()}</p>
            </div>
          ))}
        </div>
      )}
